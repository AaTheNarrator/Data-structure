program lab_1_1
    ...
    implicit none
    character(kind=CH_), allocatable :: Surnames(:, :), SortSurnames(:, :)
    character(kind=CH_), allocatable :: Positions(:, :), SortPositions(:, :)
    ...
    call write_original_list( &
        OUT_FILE, Surnames, Positions, &
        "Исходный список:", position="rewind" &
    )
