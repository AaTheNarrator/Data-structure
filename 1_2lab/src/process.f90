...
contains
   ...
   pure integer function rank_of_position(Position, SortedPosition) result(rank)
      ...
      allocate(mask(ubound(SortedPosition, 1)))
      mask = [(all(Position == SortedPosition(i, :)), i = 1, ubound(SortedPosition, 1))]
   ...
   subroutine sort_array_by_position(Surnames, Positions, SortedPosition, SortSurnames, SortPosition)
      ...
      n = ubound(Positions, 1)
      m = ubound(SortedPosition, 1)

      allocate(SortSurnames(n, SURNAME_LEN), SortPosition(n, POSITION_LEN), Counts(m), Starts(m))
